...
- **지속적인 품질 검사:** 깨진 링크, 중복 질문, 근거 없는 정밀 수치를 자동으로 점검합니다.
...
| RAG 엔지니어 | [LLM 기초](docs/01-basic-concepts/) → [RAG](docs/03-rag-system/) → [벡터 검색](docs/06-vector-index-optimization/) → [고급 RAG](docs/20-rag-advanced-optimization/) → [안전·평가](docs/09-ai-safety-evaluation/) |
| Agent 엔지니어 | [Prompt Engineering](docs/02-prompt-engineering/) → [Agent 기초](docs/05-ai-agent-basics/) → [계획·성찰](docs/22-agent-planning-reflection/) → [Multi-Agent](docs/13-multi-agent-systems/) → [MCP·Skills](docs/14-mcp-skill-systems/) |
...
| AI 안전·평가 엔지니어 | [LLM 기초](docs/01-basic-concepts/) → [Prompt Engineering](docs/02-prompt-engineering/) → [안전·평가](docs/09-ai-safety-evaluation/) → [관측 가능성](docs/23-agent-observability/) |
...
- **엔지니어링과 시스템 설계:** 안전 평가, 프로덕션 배포, Python, AI 시스템 설계, FDE.
