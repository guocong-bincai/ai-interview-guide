...
## 十九、生产环境多 Agent 编排：反模式与场景设计（Q23）

### Q23: 生产环境多 Agent 编排有哪些反模式？客服工单场景怎么拆 Agent？（2026 高频）

> 2026 年 Multi-Agent 已成为 AI Agent 岗面试标配：面试官不再问"你用没用过多 Agent"，而是直接追问"多个 Agent 怎么分工、怎么通信、状态怎么管、踩过什么坑"。能说出反模式 + 完整场景设计的候选人，和只会背架构名词的人差距明显。

<details>
<summary>💡 答案要点</summary>

**先给生产环境默认结论：** 中心化编排（Orchestrator-Worker）是生产环境的默认选项；去中心化 P2P 理论灵活，但生产环境有四个硬伤——任务分配无协调、执行顺序无保证、失败无感知、全链路不可追踪。所以大厂面试基本跳过拓扑概念，直接考"为什么生产只选 Orchestrator"。

**三大经典反模式（说出这三个词，面试官就知道你真做过）：**

| 反模式 | 现象 | 危害 | 识别方法 |
|--------|------|------|----------|
| **伪多体** | 把几个调 LLM 的函数包装成"多 Agent"，没有真实分工 | 本质是单 Agent 套壳，复杂度白加，出了问题还难排查 | 各"Agent"职责是否重叠、是否共享全部上下文 |
| **通信冗余** | 中心化架构里 Workers 绕开 Orchestrator 互相直发消息 | 可追踪性崩溃，审计链路断裂 | 看消息是否都经过编排器、是否有旁路通道 |
...
记忆 Agent（Memory）：同步全流程上下文，避免信息断层
```
...
**两个必准备的"真实坑"（面试官必追）：**
1. 上下文漂移导致任务失败——早期决策被挤出窗口，长任务质量退化（有具体 token 数字、任务长度更好）
2. 动态路由陷入无限循环——怎么发现（成本监控/轮数上限告警）、怎么修（终止条件 + 循环检测）、修完效果

**面试话术：**
> "我的 Multi-Agent 落地方案是 Orchestrator-Worker 中心化编排：规划 Agent 拆任务、Worker 无状态执行、评审 Agent 校验打回、记忆 Agent 同步上下文。生产环境我坚持三个反模式检查——不做伪多体（每个 Agent 必须有真实独立职责）、禁止 Workers 绕开编排器直连（保可追踪性）、动态路由必须设终止条件和循环检测（防无限循环烧钱）。比如客服工单场景，我拆成分类/知识库/执行/人工兜底四个 Agent，置信度低于阈值强制转人工，全流程留痕可回溯。踩过的坑是早期路由没设终止条件，两个 Agent 来回踢皮球把 token 烧爆了，后来加了最大轮数和循环检测才解决。"
