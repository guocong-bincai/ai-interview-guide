...
## 十九、RAG 生产环境实战：排查 / 评估 / 灰度 / 成本归因（Q26-Q35）

> 来源：2026 年一线 RAG 岗真实面试深挖点（王中阳公众号）。面试官原话："筛 RAG 简历都有心理阴影——很多人写，但答得好的非常少。学历无关。" 这 10 题全是"做出来之后"的工程化问题，答得上 = 真做过。
...
**第四层：系统性预防（查完还要防）**
- 建标注集：50–100 条真实问答，每次改策略都跑一遍，防止"修好 A 搞坏 B"
...
**面试加分示例：**
> "我们当时换了 bge-large-zh 之后，通用问答 Recall@5 提了 12%，但报错码类查询降了 8%，后来加了 BM25 融合才两边都稳住。"
...
**具体做法：**
1. **Shadow 模式（影子模式）：** 新策略和旧策略同时跑，新策略只记录结果不返回给用户。对比两者检索质量和生成质量，指标 OK 再切流量
2. **Canary 发布（金丝雀）：** 先给 5% 用户（或特定 user_id hash 区间）用新策略，监控核心指标（延迟、准确率、成本），无异常逐步放量 50% → 100%
3. **一键回滚：** 配置中心存策略版本号，回滚就是改一个 key，不需要重新部署代码，**30 秒内完成**
4. **质量门禁：** 灰度期间准确率低于基线 X% 或延迟超过 Y ms，自动触发告警并建议回滚

**面试话术：**
> "灰度 = Shadow 验证 + Canary 放量 + 一键回滚 + 质量门禁。新策略先在 Shadow 模式跟跑只记录不返回，指标达标后切 5% 流量，监控延迟/准确率/成本没问题再逐步放量；配置中心存策略版本号，回滚改一个 key 就行，30 秒内完成；灰度期间设质量门禁，低于基线自动告警建议回滚。能说出 Shadow 和 Canary 的区别、提到配置热加载，就已经超过 90% 的候选人了。"

### Q29: chunk 大小怎么定？重叠（overlap）为什么重要？

**chunk 大小没有万能值，取决于文档类型和检索粒度：**
- 短文本（FAQ、客服对话）：100–200 字，本身信息密度高
- 长文档（技术手册、论文）：300–500 字，平衡语义完整性和检索精度
- 法律/合同类：按条款切更合适，不宜硬切字数

**overlap 为什么重要：**
没有 overlap，一个完整意思刚好被切成两半时，边界处信息就丢了。例如"因此该方法不建议在生产环境使用"——前半句在 chunk A 尾部、后半句在 chunk B 头部，没有 overlap 两个 chunk 都读不出完整警告含义。一般 overlap 取 chunk 长度的 **10%–20%**。

**怎么验证大小合不合理：**
- 抽样检查 chunk 质量（边界是否截断关键信息）
- 跑 Recall@K 对比不同 chunk size 下的检索表现
- 用人工标注集做 A/B 对比

**面试加分示例：**
> "我们试过 200/400/600 三种 size，400+80 overlap 在我们的语料上 Recall@5 最高（0.88 vs 0.72 vs 0.81），边界截断率从 23% 降到 6%。"

### Q30: 为什么用混合检索？纯向量不行吗？

**一句话答案：** 纯向量检索在"语义相似但不精确匹配"场景很强，但在"关键词精确匹配"场景（专有名词、报错码、版本号、缩写）会漏。混合检索 = 向量（理解语义）+ BM25（精确匹配），取长补短。

**向量的盲区：** 用户搜「ECONNRESET 怎么处理」，向量可能召回"网络连接相关的各种问题"，但漏掉包含精确关键字 ECONNRESET 的那条文档——embedding 把它编码成语义向量后，精确字符匹配信息丢失了。

**BM25 的盲区：** 用户搜「数据库连接超时怎么办」，BM25 只匹配字面词，可能漏掉「connection timeout」「链接超时」等同义表述的文档。

**融合方式：**
- 常用 **RRF（Reciprocal Rank Fusion）**：对多个检索结果按排名倒数融合，公式简单效果好
- 也可以用加权线性组合
- 两种结果各取 top N，融合后再 rerank 取 top K

**量化效果示例：**
> "混合检索比纯向量的 Recall@5 从 0.62 提升到 0.88，bad case 率下降约 40%。"

### Q31: 重排（rerank）怎么做？为什么能提升效果？

**原理：**
- 检索阶段（向量/BM25）追求"快和广"，用的是**双塔模型**，query 和 doc 分开编码，精度有限
- 重排阶段用 **cross-encoder** 把 query 和每个 candidate 拼在一起精细打分，精度高但慢
- 所以只对检索出的 top N（如 20 条）做重排，再取 top K（如 5 条）送给模型生成

**模型选择：** bge-reranker / cross-encoder 系列，中文场景 bge-reranker-v2-m3 效果好。

**为什么有效：** 检索阶段可能把"看起来相关但其实不对"的内容排前面（比如都是数据库，一个是 MySQL 一个是 PostgreSQL）。重排能精排"query 和这个 document 到底有多相关"。

**量化效果示例：**
> "加 rerank 后，端到端准确率从 71% 提升到 84%（基于 80 条标注集）。"

**面试话术：**
> "检索是双塔求快求广，重排是 cross-encoder 求准——把 top 20 候选逐一精排取 top 5，中文场景用 bge-reranker-v2-m3。本质是解决'语义相似 ≠ 真正相关'的问题，我们实测端到端准确率从 71% 提到 84%。"

### Q32: RAG 成本怎么控制？怎么归因？

**请求级归因：** 每次请求记一笔——user_id / 业务线 / 接口名 / 模型名 / input_tokens / output_tokens / latency / cost。按业务线聚合，谁烧钱一目了然。

**降本手段：**
- **缓存：** 高频相同或相似 query 直接返回缓存结果，命中率 30–50% 能砍掉近一半推理费用
- **模型路由：** 简单问题走 7B 小模型（便宜），复杂问题才路由到旗舰模型（贵但准），省 60–70% 成本
- **量化部署：** INT4 跑开源模型，显存砍半
- **上下文裁剪：** 只拼最相关的 topK 文档进 prompt，别把 20 条全塞进去（token 直接烧钱）

**面试加分场景：** 老板问"为什么 AI 账单涨了"——打开归因看板："A 业务量涨了 40%、B 业务新上了 Agent 功能、C 业务 max_tokens 设太大导致单次成本是正常的 3 倍"——有数据、有归因、有行动建议。

### Q33: 知识库更新怎么处理？增量还是全量重建？

**增量更新：** 监听文档变更事件，只对新增/修改的文档做 embedding 并写入向量库，删除的做标记或物理删除。适合文档量大、更新频率低的场景。

**全量重建：** 定期（如每天凌晨）全量重新 embedding + 写入。简单粗暴但可靠，适合中小规模（万级文档以下）。

**实际选择（推荐答案）：** "增量为主 + 定期全量校验"——平时增量更新保证实时性，每周一次全量跑一遍检测是否有遗漏或漂移。

**注意点：** 增量更新要处理好「同文档多次修改」（避免重复入库）和「删除级联」（原文档删了，旧的 embedding 也要清）。

### Q34: Agent 场景下工具调用怎么兜底？

**schema 校验：** 工具定义必须写清参数类型、枚举值、必填项。模型返回的参数要先过 schema 校验，不合格的直接拒绝调用，而不是传给工具炸掉。

**不存在工具的处理：** 模型可能调用不存在的工具名，要做白名单校验，不在名单里的返回错误提示让模型重新选择。

**超时与重试：** 工具调用设置超时（如 10s），超时后返回错误而非无限等待。非幂等工具不做自动重试，幂等的最多重试 2 次。

**人在环（HITL）：** 关键操作（删数据、发通知、转账）必须有人工确认环节，不能让模型自主执行。

**上下文爆炸防护：** 多轮工具调用后 context 会膨胀，用摘要压缩历史、只保留最近 N 轮 + 当前任务相关上下文。

### Q35: 你做的 RAG 项目最大挑战是什么？（开放题）

**开放题要点：** 面试官想听真实踩坑经历。要有**具体场景 + 具体动作 + 量化结果 + 反思**，别说"最大挑战是学习新技术"这种空话。

**参考答法：**
> "最大挑战是'检索质量的可观测性'。刚上线时用户反馈'有时候准有时候不准'，但我们没有任何数据支撑——不知道是哪类问题不准、不准的时候检索到了什么。后来搭建了全链路 trace（query → 检索文档 → 生成回答 → 用户反馈），建了 80 条标注集作为回归基准，才有了'改策略 → 跑评测 → 看指标 → 决定是否放量'的闭环。这个过程花了大概两周，但之后每次迭代效率提升了至少 3 倍。"

**面试话术总结：**
> "答这类题要落地：具体踩过什么坑（可观测性缺失）→ 具体怎么补（全链路 trace + 标注集回归）→ 量化收益（迭代效率 ×3）。面试官要的是'你被真实流量教育过'的证据。"


---

*版本: v3.151 | 更新: 2026-08-12 | 新增 RAG 生产环境实战 10 题(Q26-Q35：分层排查/跷跷板效应/灰度发布/成本归因/增量更新/工具兜底，源自王中阳公众号) | by 二狗子 🐕*
