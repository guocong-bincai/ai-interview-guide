...
### Q19: 2026年 RAG 有哪些新范式？为什么传统 RAG 正在失效？企业如何判断 RAG 项目方向？
...
### Q24: 什么是混合检索？为什么 2026 年企业级 RAG 必须用"稀疏 + 密集"双路检索？BM25 和 RRF 融合原理是什么？
...
## 十九、RAG 生产环境实战：排查 / 评估 / 灰度 / 复盘（Q26-Q29）
...
**表达模板：**
> “在我们的评测集中，变更后 `[问题类型 A]` 改善，但 `[问题类型 B]` 回退。我们检查 bad case 后确认原因是 `[原因]`，通过 `[混合检索/路由/数据修复]` 处理，并用相同切片重新评估。”请只填入自己的真实结果。
